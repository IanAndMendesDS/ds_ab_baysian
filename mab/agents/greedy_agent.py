...
for episode in range(episodes):
    ...
    for trial in range(trials):
        # agent - escolha
        if eps_init ==0:
            bandit_machine = 0 # exploração - Escolhendo a primeira pagina

        elif eps_init == 1: 
            bandit_machine = 1 # exploração - Escolhendo a segunda pagina

        elif eps_init == 2:
            bandit_machine = reward_array / bandit_array # exploração - Escolhendo a pagina com maior probabilidade de recompensa
            max_prob_reward = np.where(reward_array == np.max(reward_array))[0]
            bandit_machine = max_prob_reward[0]   
        else:
            eps_init += 1
        
        # increasing eps_init to change the strategy of the agent
        eps_init += 1
